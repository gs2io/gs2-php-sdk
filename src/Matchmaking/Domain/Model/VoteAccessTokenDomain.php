...
class VoteAccessTokenDomain {
    ...
    public function getBallot(
        \Gs2\Matchmaking\Request\GetBallotRequest $request
    ): \Gs2\Matchmaking\Result\GetBallotResult {
        $request->setNamespaceName($this->namespaceName);
        $request->setAccessToken($this->accessToken !== null ? $this->accessToken->getToken() : null);
        $request->setRatingName($this->ratingName);
        $request->setGatheringName($this->gatheringName);
        /**
         * @var \Gs2\Matchmaking\Result\GetBallotResult
         */
        $r = $this->client->getBallot(
            $request
        );
        return $r;
    }
