...
class CreateMatchSessionResult implements IResult {
    ...
    public static function fromJson(?array $data): ?CreateMatchSessionResult {
        if ($data === null) {
            return null;
        }
        return (new CreateMatchSessionResult())
            ->withItem(array_key_exists('item', $data) && $data['item'] !== null ? MatchSession::fromJson($data['item']) : null);
    }
