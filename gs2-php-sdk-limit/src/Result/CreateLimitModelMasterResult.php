...
class CreateLimitModelMasterResult implements IResult {
	/** @var LimitModelMaster 作成した回数制限の種類マスター */
	private $item;

	/**
	 * 作成した回数制限の種類マスターを取得
	 *
	 * @return LimitModelMaster|null 回数制限の種類マスターを新規作成
	 */
	 ...
	 * 作成した回数制限の種類マスターを設定
	 *
	 * @param LimitModelMaster|null $item 回数制限の種類マスターを新規作成
	 */
	public function setItem(?LimitModelMaster $item) {
		$this->item = $item;
	}

    public static function fromJson(array $data): CreateLimitModelMasterResult {
        $result = new CreateLimitModelMasterResult();
