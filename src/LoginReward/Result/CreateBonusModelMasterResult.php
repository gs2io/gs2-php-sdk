...
class CreateBonusModelMasterResult implements IResult {
	...
	public function withItem(?BonusModelMaster $item): CreateBonusModelMasterResult {
		$this->item = $item;
		return $this;
	}

    public static function fromJson(?array $data): ?CreateBonusModelMasterResult {
        if ($data === null) {
            return null;
        }
        return (new CreateBonusModelMasterResult())
            ->withItem(array_key_exists('item', $data) && $data['item'] !== null ? BonusModelMaster::fromJson($data['item']) : null);
    }
