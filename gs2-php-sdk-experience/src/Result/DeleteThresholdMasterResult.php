...
class DeleteThresholdMasterResult implements IResult {
	/** @var ThresholdMaster 削除したランクアップ閾値マスター */
	private $item;

	/**
	 * 削除したランクアップ閾値マスターを取得
	 *
	 * @return ThresholdMaster|null ランクアップ閾値マスターを削除
	 */
	 ...
	 * 削除したランクアップ閾値マスターを設定
	 *
	 * @param ThresholdMaster|null $item ランクアップ閾値マスターを削除
	 */
	public function setItem(?ThresholdMaster $item) {
		$this->item = $item;
	}

    public static function fromJson(array $data): DeleteThresholdMasterResult {
        $result = new DeleteThresholdMasterResult();
