...
class CreateCounterModelMasterResult implements IResult {
	/** @var CounterModelMaster 作成したカウンターの種類マスター */
	private $item;

	/**
	 * 作成したカウンターの種類マスターを取得
	 *
	 * @return CounterModelMaster|null カウンターの種類マスターを新規作成
	 */
	 ...
	 * 作成したカウンターの種類マスターを設定
	 *
	 * @param CounterModelMaster|null $item カウンターの種類マスターを新規作成
	 */
	public function setItem(?CounterModelMaster $item) {
		$this->item = $item;
	}

    public static function fromJson(array $data): CreateCounterModelMasterResult {
        $result = new CreateCounterModelMasterResult();
