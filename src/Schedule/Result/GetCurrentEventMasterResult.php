...
/**
 * 現在有効なイベントスケジュールマスターを取得します のレスポンスモデル
 *
...
class GetCurrentEventMasterResult implements IResult {
	 ...
	 *
	 * @return CurrentEventMaster|null 現在有効なイベントスケジュールマスターを取得します
	 */
	 ...
	 *
	 * @param CurrentEventMaster|null $item 現在有効なイベントスケジュールマスターを取得します
	 */
	public function setItem(?CurrentEventMaster $item) {
		$this->item = $item;
	}

    public static function fromJson(array $data): GetCurrentEventMasterResult {
        $result = new GetCurrentEventMasterResult();
