...
class CountExecuteStampTaskLogResult implements IResult {
	/** @var ExecuteStampTaskLogCount[] スタンプタスク実行ログ集計のリスト */
	private $items;
	...
	/**
	 * スタンプタスク実行ログ集計のリストを取得
	 *
	 * @return ExecuteStampTaskLogCount[]|null スタンプタスク実行ログの一覧を取得
	 */
	...
	/**
	 * スタンプタスク実行ログ集計のリストを設定
	 *
	 * @param ExecuteStampTaskLogCount[]|null $items スタンプタスク実行ログの一覧を取得
	 */
    ...
    public static function fromJson(array $data): CountExecuteStampTaskLogResult {
        $result = new CountExecuteStampTaskLogResult();
        $result->setItems(array_map(
                function ($v) {
                    return ExecuteStampTaskLogCount::fromJson($v);
                },
