...
class GetStoreContentModelResult implements IResult {
    ...
    public static function fromJson(?array $data): ?GetStoreContentModelResult {
        if ($data === null) {
            return null;
        }
        return (new GetStoreContentModelResult())
            ->withItem(array_key_exists('item', $data) && $data['item'] !== null ? StoreContentModel::fromJson($data['item']) : null);
    }
