...
class DeleteRecoverValueTableMasterResult implements IResult {
	/** @var RecoverValueTableMaster 削除したスタミナ回復量テーブルマスター */
	private $item;

	/**
	 * 削除したスタミナ回復量テーブルマスターを取得
	 *
	 * @return RecoverValueTableMaster|null スタミナ回復量テーブルマスターを削除
	 */
	 ...
	 *
	 * @param RecoverValueTableMaster|null $item スタミナ回復量テーブルマスターを削除
	 */
	public function setItem(?RecoverValueTableMaster $item) {
		$this->item = $item;
	}

    public static function fromJson(array $data): DeleteRecoverValueTableMasterResult {
        $result = new DeleteRecoverValueTableMasterResult();
