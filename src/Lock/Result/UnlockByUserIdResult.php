...
class UnlockByUserIdResult implements IResult {
	 ...
	 * ミューテックスを取得
	 *
	 * @return Mutex|null ユーザIDを指定してミューテックスを解放
	 */
	 ...
	 * ミューテックスを設定
	 *
	 * @param Mutex|null $item ユーザIDを指定してミューテックスを解放
	 */
	public function setItem(?Mutex $item) {
		$this->item = $item;
	}

    public static function fromJson(array $data): UnlockByUserIdResult {
        $result = new UnlockByUserIdResult();
