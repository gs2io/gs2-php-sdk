...
class CreateGlobalMessageMasterResult implements IResult {
	/** @var GlobalMessageMaster 作成した全ユーザに向けたメッセージ */
	private $item;

	/**
	 * 作成した全ユーザに向けたメッセージを取得
	 *
	 * @return GlobalMessageMaster|null 全ユーザに向けたメッセージを新規作成
	 */
	 ...
	 *
	 * @param GlobalMessageMaster|null $item 全ユーザに向けたメッセージを新規作成
	 */
	public function setItem(?GlobalMessageMaster $item) {
		$this->item = $item;
	}

    public static function fromJson(array $data): CreateGlobalMessageMasterResult {
        $result = new CreateGlobalMessageMasterResult();
