...
class CountAccessLogResult implements IResult {
	/** @var AccessLogCount[] アクセスログ集計のリスト */
	private $items;
	 ...
	 * アクセスログ集計のリストを取得
	 *
	 * @return AccessLogCount[]|null アクセスログの一覧を取得
	 */
	 ...
	 * アクセスログ集計のリストを設定
	 *
	 * @param AccessLogCount[]|null $items アクセスログの一覧を取得
	 */
    ...
    public static function fromJson(array $data): CountAccessLogResult {
        $result = new CountAccessLogResult();
        $result->setItems(array_map(
                function ($v) {
                    return AccessLogCount::fromJson($v);
                },
