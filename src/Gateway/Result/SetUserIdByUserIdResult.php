...
class SetUserIdByUserIdResult implements IResult {
    ...
    public static function fromJson(?array $data): ?SetUserIdByUserIdResult {
        if ($data === null) {
            return null;
        }
        return (new SetUserIdByUserIdResult())
            ->withItem(array_key_exists('item', $data) && $data['item'] !== null ? WebSocketSession::fromJson($data['item']) : null);
    }
